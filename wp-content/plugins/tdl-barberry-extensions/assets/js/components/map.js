...
jQuery(document).ready(function($){
	...
	window.mapAPI_Loaded = function() {
		...
		for(var i = 0; i < $('.barberry-google-map').length; i++) {
			 infoWindows[i] = [];
		}

		$('.barberry-google-map').each(function(i){
			...
			barberryMarkerColor = $(this).attr('data-marker-color');
	...
	if(typeof google === 'object' && typeof google.maps === 'object') {
 		...
 		if(barberry_scripts_vars.mapApiKey.length > 0) {
 			$.getScript('https://maps.google.com/maps/api/js?sensor=false&key='+barberry_scripts_vars.mapApiKey+'&callback=mapAPI_Loaded');
 		} else {
		  ...
		  $('.map-marker-list.'+map_id).each(function(){
		        ...
		        var enableAnimation = $('#'+map_id).attr('data-enable-animation');
		      	...
		      	$(this).find('.map-marker').each(function(i){
					...
					//eva marker 
					if($('#'+map_id).is('[data-marker-style="barberry"]')) {
