 ...
 */
class Barberry_Add_Custom_Fonts {
	...
	public function init_hooks() {
		...
		add_filter( 'barberry/kirki_font_choices', array( $this, 'add_custom_fonts' ) );
	}
	...
	public function get_custom_fonts() {
		...
		update_option( 'barberry-custom-fonts', Bsf_Custom_Fonts_Taxonomy::get_fonts() );
	}
	...
	public function prepare_custom_fonts_acf() {
		...
		if ( barberry_have_rows( 'custom_fonts', 'option' ) ) {
			
			while ( barberry_have_rows( 'custom_fonts', 'option' ) ) {
				...
				if ( ! empty( $font_name ) && barberry_have_rows( 'font_files', 'option' ) ) {
					while ( barberry_have_rows( 'font_files', 'option' ) ) {
	...
	public function prepare_custom_fonts() {
		...
		$fonts = get_option( 'barberry-custom-fonts' );
	...
	public function add_custom_fonts( $custom_choice ) {
		...
		$custom_choice['families']['custom'] = array(
			'text'     => esc_attr__( 'Custom Fonts', 'barberry' ),
	...
	public function print_custom_css() {
		...
		if ( ! empty (self::$css_to_print) ) {
			add_action('wp_enqueue_scripts', function() {
				wp_add_inline_style( 'barberry-app', trim( self::$css_to_print ) );
			}, 30);
		}
