 ...
 * @package Barberry
 */
class Barberry_Mega_Menu_Walker extends Walker_Nav_Menu {
	/**
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		if ( ! $this->in_mega ) {
			$output .= "\n$indent<ul class=\"dropdown-submenu menu\">\n";
		} else {
			if ( $depth == 0 ) {
				$output .= "\n$indent<ul class=\"dropdown-submenu menu\">\n$indent<li>\n$indent<div\n$this->style class=\"mega-menu-content\">\n$indent<div class=\"grid-x\">\n";
			} elseif ( $depth == 1 ) {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$item_is_mega         = apply_filters( 'barberry_menu_item_mega', get_post_meta( $item->ID, 'tamm_menu_item_mega', true ), $item->ID );
		...
		if ( in_array( 'menu-item-has-children', $classes ) ) {
			if ( ! $depth || ( $depth && ! intval($this->in_mega)) ) {
				$classes[] = 'dropdown is-dropdown-submenu-parent';
			}
		...
		if ( $depth == 1 && intval($this->in_mega) ) {
			...
			$class_names = ' class="cell medium-' . $this->column . '"';
		...
		if ( $item_hot || $item_new || $item_trending ) {
			...
			if ( $item_hot ) {
				$badge[] = '<span class="hot-badge">' . esc_html__( 'Hot', 'barberry' ) . '</span>';
			}
			if ( $item_new ) {
				$badge[] = '<span class="new-badge">' . esc_html__( 'New', 'barberry' ) . '</span>';
			}
			if ( $item_trending ) {
				$badge[] = '<span class="trending-badge">' . esc_html__( 'Trending', 'barberry' ) . '</span>';
			}
			$badge[] = '</span>';
		}
		...
		if ( $depth == 1 && intval($this->in_mega) ) {
			...
			$item_output .= $args->link_before . $title . $args->link_after . implode( $badge );
			$item_output .= '</a>';
