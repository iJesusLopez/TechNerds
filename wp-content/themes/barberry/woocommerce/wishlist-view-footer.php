    ...
    <div class="yith_wcwl_wishlist_footer">
        <?php if ( $count && $show_cb ) : ?>
            ...
            <div class="yith_wcwl_wishlist_bulk_action">
                <label for="bulk_actions"><?php echo apply_filters( 'yith_wcwl_wishlist_bulk_actions_label', __( 'Apply this action to all the selected items:', 'barberry' ) ) ?></label>
                <select name="bulk_actions" id="bulk_actions">
                    <option value="add_to_cart"><?php _e( 'Add to cart', 'barberry' ) ?></option>

                    <?php if ( $is_user_owner ): ?>
                        <option value="delete"><?php _e( 'Remove from wishlist', 'barberry' ) ?></option>
                        ...
                        <?php if( $available_multi_wishlist && count( $users_wishlists ) > 1 ): ?>
                            ...
                            ?>
                                <option value="<?php echo esc_attr( $wl->get_token() ) ?>"><?php printf( __( 'Move to %s', 'barberry' ), $wl->get_formatted_name() ) ?></option>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    <?php endif; ?>
                </select>
                <input type="submit" name="apply_bulk_actions" value="<?php _e( 'Apply', 'barberry' ) ?>"/>
            </div>
        <?php endif; ?>
	    ...
	    <?php if( $count && $show_update ) : ?>
            ...
            <div class="yith_wcwl_wishlist_update">
                <input type="submit" name="update_wishlist" value="<?php _e( 'Update', 'barberry' ) ?>"/>
            </div>
	    <?php endif; ?>
        ...
        <?php if( $count && ( $show_ask_estimate_button || $enable_add_all_to_cart ) ): ?>
            <div class="yith_wcwl_footer_additional_action">
                <?php if( $count && $show_ask_estimate_button ): ?>
                    ...
                    <a href="<?php echo esc_attr( $additional_info || ! is_user_logged_in() ) ? '#ask_an_estimate_popup' : $ask_estimate_url ?>" class="<?php echo esc_attr( $ask_an_estimate_classes ) ?> ask-an-estimate-button" <?php echo esc_attr( $additional_info ) ? 'data-rel="prettyPhoto[ask_an_estimate]"' : '' ?> >
                        <?php echo apply_filters( 'yith_wcwl_ask_an_estimate_icon', $ask_an_estimate_icon )?>
                        <?php echo apply_filters( 'yith_wcwl_ask_an_estimate_text', __( 'Ask for an estimate', 'barberry' ) ) ?>
                    </a>
                <?php endif; ?>

                <?php if( $count && $enable_add_all_to_cart ): ?>
                    <!-- Add all to cart button -->
                    <input type="submit" name="add_all_to_cart" value="<?php _e( 'Add all to cart', 'barberry' ) ?>" />
                <?php endif; ?>
            </div>
        <?php endif; ?>
    </div>
